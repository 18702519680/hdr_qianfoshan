...
public class ImpalaUtil {
	 ...
	 * @return
	 */
	public static Connection createImpalaConnection() {
		...
		String hive_url = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "impala.conn");
		String hive_user = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "impala.user");
		String hive_pass = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "impala.password");
		...
		try {
			...
			e.printStackTrace();
			throw new ApplicationException("impala驱动加载失败！");
		} catch (SQLException e) {
			e.printStackTrace();
			throw new ApplicationException("impala连接创建失败！");
		}
		return conn;
