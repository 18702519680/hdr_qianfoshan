...
/**
 * @Description 类描述：治疗信息（放疗用药记录） 上传每次归档后，当次住院的放疗时使用的药物信息。
 * @author 吴黎兵
 * @Date 2020年6月11日 方法名：radiotherapyDrugRecord 版本号：1.0.1
 * @modify 修改记录：
 * 
 */
public class REPORT12 {
	protected static Logger logger = LoggerFactory.getLogger(REPORT12.class);
	private static String CONFIG_FILE_NAME = "report2.properties";
	private static String function_name = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "radiotherapyDrugRecord");
	private static String version = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"radiotherapyDrugRecord_version");
	...
	public static JSONArray getRadiotherapyDrugRecordJsonInfo(Map<String, String> map) {
		...
		try {
			...
			json.put("ncRadiotherapyTime", map.get("NCRADIOTHERAPYTIME"));// 放疗时间
			json.put("ncRadiotherapyDrug", map.get("NCRADIOTHERAPYDRUG"));// 放疗药物
	...
	public static void postRadiotherapyDrugRecord(String startDate, String endDate) {
		 ...
		 */
		listSumm = Xmlutil
				.formatList(HbaseCURDUtils.findByRowkey(radiotherapydrugrecord_tablename, start, end, filters));
		...
		if (listSumm.size() > 0) {
			...
			try {
				for (int i = 0; i < 10; i++) {
					...
					json = getRadiotherapyDrugRecordJsonInfo(mapInfo);
					jsonall.addAll(json);
				}
				// 反馈报文
	...
	public static void postRadiotherapyDrugRecordByrowkey(String rowkey) {
		List<Map<String, String>> listSumm = new ArrayList<Map<String, String>>();
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		...
		listSumm = Xmlutil
				.formatList(HbaseCURDUtilsToCDA.findByRowkeyList(radiotherapydrugrecord_tablename, listGet, columns));
		JSONArray json = new JSONArray();
		JSONArray jsonall = new JSONArray();
		JSONObject jsonallarr = new JSONObject();
		if (listSumm.size() > 0) {
