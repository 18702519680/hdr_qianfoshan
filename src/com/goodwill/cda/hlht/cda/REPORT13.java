...
/**
 * @Description 类描述：治疗信息（靶向治疗用药记录） 上传每次归档后，当次住院的靶向治疗时使用的药物信息。
 * @author 吴黎兵
 * @Date 2020年6月11日 方法名：targettherapyDrugRecord 版本号：1.0.1
 * @modify 修改记录：
 * 
 */
public class REPORT13 {
	protected static Logger logger = LoggerFactory.getLogger(REPORT13.class);
	private static String CONFIG_FILE_NAME = "report2.properties";
	private static String function_name = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "targettherapyDrugRecord");
	private static String version = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"targettherapyDrugRecord_version");
	private static String targettherapydrugrecord_tablename = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"targettherapyDrugRecord_tablename");

	public static void main(String[] args) throws Exception {
	}

	public static JSONArray getTargettherapyDrugRecordJsonInfo(Map<String, String> map) {
		...
		try {
			...
			json.put("ncTargettherapyNo", map.get("NCTARGETTHERAPYNO"));// 靶向治疗记录编号
			json.put("ncTargettherapyTime", map.get("NCTARGETTHERAPYTIME"));// 靶向治疗时间
			json.put("ncTargettherapyDrug", map.get("NCTARGETTHERAPYDRUG"));// 靶向治疗药物
	...
	public static void postTargettherapyDrugRecord(String startDate, String endDate) {
		 ...
		 */
		listSumm = Xmlutil
				.formatList(HbaseCURDUtils.findByRowkey(targettherapydrugrecord_tablename, start, end, filters));
		...
		if (listSumm.size() > 0) {
			...
			try {
				for (int i = 0; i < 10; i++) {
					...
					json = getTargettherapyDrugRecordJsonInfo(mapInfo);
					jsonall.addAll(json);
				}
				// 反馈报文
	...
	public static void postTargettherapyDrugRecordByrowkey(String rowkey) {
		List<Map<String, String>> listSumm = new ArrayList<Map<String, String>>();
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		...
		listSumm = Xmlutil
				.formatList(HbaseCURDUtilsToCDA.findByRowkeyList(targettherapydrugrecord_tablename, listGet, columns));
		JSONArray json = new JSONArray();
		JSONArray jsonall = new JSONArray();
		JSONObject jsonallarr = new JSONObject();
		if (listSumm.size() > 0) {
