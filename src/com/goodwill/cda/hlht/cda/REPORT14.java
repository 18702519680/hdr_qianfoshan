...
/**
 * @Description 类描述：治疗信息（生物治疗用药记录） 上传每次归档后，当次住院的生物治疗时使用的药物信息。
 * @author 吴黎兵
 * @Date 2020年6月11日 方法名：biotherapyDrugRecord 版本号：1.0.1
 * @modify 修改记录：
 * 
 */
public class REPORT14 {
	protected static Logger logger = LoggerFactory.getLogger(REPORT14.class);
	private static String CONFIG_FILE_NAME = "report2.properties";
	private static String function_name = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "biotherapyDrugRecord");
	private static String version = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "biotherapyDrugRecord_version");
	private static String biotherapydrugrecord_tablename = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"biotherapyDrugRecord_tablename");

	public static void main(String[] args) throws Exception {
	}

	public static JSONArray getBiotherapyDrugRecordJsonInfo(Map<String, String> map) {
		...
		try {
			...
			json.put("ncBiotherapyNo", map.get("NCBIOTHERAPYNO"));// 生物治疗记录编号
			json.put("ncBiotherapyTime", map.get("NCBIOTHERAPYTIME"));// 生物治疗时间
			json.put("ncBiotherapyDrug", map.get("NCBIOTHERAPYDRUG"));// 生物治疗药物
	...
	public static void postBiotherapyDrugRecord(String startDate, String endDate) {
		 ...
		 */
		listSumm = Xmlutil.formatList(HbaseCURDUtils.findByRowkey(biotherapydrugrecord_tablename, start, end, filters));
		...
		if (listSumm.size() > 0) {
			...
			try {
				for (int i = 0; i < 10; i++) {
					...
					json = getBiotherapyDrugRecordJsonInfo(mapInfo);
					jsonall.addAll(json);
				}
				// 反馈报文
	...
	public static void postBiotherapyDrugRecordByrowkey(String rowkey) {
		List<Map<String, String>> listSumm = new ArrayList<Map<String, String>>();
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		...
		listSumm = Xmlutil
				.formatList(HbaseCURDUtilsToCDA.findByRowkeyList(biotherapydrugrecord_tablename, listGet, columns));
		JSONArray json = new JSONArray();
		JSONArray jsonall = new JSONArray();
		JSONObject jsonallarr = new JSONObject();
		if (listSumm.size() > 0) {
