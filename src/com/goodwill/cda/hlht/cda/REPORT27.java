...
/**
 * @Description 类描述：病理记录（分子病理） 上传每次归档后，当次住院的分子病理详细信息。
 * @author 吴黎兵
 * @Date 2020年6月11日 方法名：molecularBiologyPathology 版本号：1.0.0
 * @modify 修改记录：
 * 
 */
public class REPORT27 {
	protected static Logger logger = LoggerFactory.getLogger(REPORT27.class);
	...
	private static String function_name = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"molecularBiologyPathology");
	private static String version = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"molecularBiologyPathology_version");
	private static String molecularbiologypathology_tablename = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"molecularBiologyPathology_tablename");

	public static void main(String[] args) throws Exception {
	}

	public static JSONArray getMolecularBiologyPathologyJsonInfo(Map<String, String> map) {
		...
		try {
			...
			json.put("ncDetectionSite", map.get("NCDETECTIONSITE"));// 检测位点
			json.put("ncDetectionResult", map.get("NCDETECTIONRESULT"));// 检测结果
	...
	public static void postMolecularBiologyPathology(String startDate, String endDate) {
		 ...
		 */
		listSumm = Xmlutil
				.formatList(HbaseCURDUtils.findByRowkey(molecularbiologypathology_tablename, start, end, filters));
		...
		if (listSumm.size() > 0) {
			...
			try {
				for (int i = 0; i < listSumm.size(); i++) {
					...
					json = getMolecularBiologyPathologyJsonInfo(mapInfo);
					jsonall.addAll(json);
				}
				// 反馈报文
	...
	public static void postMolecularBiologyPathologyByrowkey(String rowkey) {
		List<Map<String, String>> listSumm = new ArrayList<Map<String, String>>();
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		...
		listSumm = Xmlutil.formatList(
				HbaseCURDUtilsToCDA.findByRowkeyList(molecularbiologypathology_tablename, listGet, columns));
		JSONArray json = new JSONArray();
		JSONArray jsonall = new JSONArray();
		JSONObject jsonallarr = new JSONObject();
		if (listSumm.size() > 0) {
