...
/**
 * @Description 类描述：治疗信息（放疗记录） 上传每次归档后，当次住院的放疗信息。
 * @author 吴黎兵
 * @Date 2020年6月11日 方法名：radiotherapyRecord 版本号：1.0.2
 * @modify 修改记录：
 * 
 */
public class REPORT11 {
	protected static Logger logger = LoggerFactory.getLogger(REPORT11.class);
	private static String CONFIG_FILE_NAME = "report2.properties";
	private static String function_name = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "radiotherapyRecord");
	private static String version = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME, "radiotherapyRecord_version");
	private static String radiotherapyrecord_tablename = PropertiesUtils.getPropertyValue(CONFIG_FILE_NAME,
			"radiotherapyRecord_tablename");
	...
	public static JSONArray getChemotherapyDrugRecordJsonInfo(Map<String, String> map) {
		...
		try {
			...
			json.put("ncRadiotherapyStartTime", map.get("NCRADIOTHERAPYSTARTTIME"));// 放疗开始时间
			json.put("ncRadiotherapyEndTime", map.get("NCRADIOTHERAPYENDTIME"));// 放疗结束时间
			json.put("ncPreOrPostOperation", map.get("NCPREORPOSTOPERATION"));// 与手术关系
			json.put("ncRadiotherapyPlan", map.get("NCRADIOTHERAPYPLAN"));// 放疗方案名称
			json.put("ncRadiotherapyContent", map.get("NCRADIOTHERAPYCONTENT"));// 放疗方案内容
			json.put("ncTargetSite", map.get("NCTARGETSITE"));// 靶区部位
			json.put("ncFieldAmount", map.get("NCFIELDAMOUNT"));// 射野数
			json.put("ncDosageCgy", map.get("NCDOSAGECGY"));// 分次剂量数cGy
			json.put("ncTotalDosageCgy", map.get("NCTOTALDOSAGECGY"));// 累计剂量数cGy
	...
	public static void postChemotherapyDrugRecord(String startDate, String endDate) {
		 ...
		 */
		listSumm = Xmlutil.formatList(HbaseCURDUtils.findByRowkey(radiotherapyrecord_tablename, start, end, filters));
	...
	public static void postChemotherapyDrugRecordByrowkey(String rowkey) {
		List<Map<String, String>> listSumm = new ArrayList<Map<String, String>>();
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		...
		listSumm = Xmlutil
				.formatList(HbaseCURDUtilsToCDA.findByRowkeyList(radiotherapyrecord_tablename, listGet, columns));
		JSONArray json = new JSONArray();
		JSONArray jsonall = new JSONArray();
		JSONObject jsonallarr = new JSONObject();
		if (listSumm.size() > 0) {
